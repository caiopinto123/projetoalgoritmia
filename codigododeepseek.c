...
void salvarParaHistorico(char **tabuleiro, int linhas, int colunas) {
    if (historicoPos < historicoCount - 1) {
        historicoCount = historicoPos + 1;
    }
    
    if (historicoCount >= MAX_HISTORICO) {
        for (int i = 0; i < linhas; i++) {
            free(historicoTabuleiros[0][i]);
        }
        free(historicoTabuleiros[0]);
        for (int i = 0; i < historicoCount - 1; i++) {
            historicoTabuleiros[i] = historicoTabuleiros[i + 1];
        }
        historicoCount--;
    }
    
    historicoTabuleiros[historicoCount] = criarTabuleiro(linhas, colunas);
    for (int i = 0; i < linhas; i++) {
        for (int j = 0; j < colunas; j++) {
            historicoTabuleiros[historicoCount][i][j] = tabuleiro[i][j];
        }
    }
    historicoPos = historicoCount;
    historicoCount++;
}
...
int main(void) {
    ...
    while(sair2 == 's') {
        ...
        if (opcao == 0) {
            ...
            for (int i = 0; i < MAX_HISTORICO; i++) {
                if (historicoTabuleiros[i]) {
                    liberarTabuleiro(historicoTabuleiros[i], linhas);
                }
            }
            free(historicoTabuleiros);
            return 0;
        }
                ...
                if (total == 0) {
                    ...
                    if (escolha == 0) {
                        ...
                        if (!fp) {
                            ...
                            historicoCount = 0;
                            historicoPos = -1;
                            salvarParaHistorico(tabuleiro, linhas, colunas);
                        }
                    }
                }
            }
        ...
        if (opcao == 1) {
            ...
            tabuleiro = criarTabuleiro(linhas, colunas);
            tabuleiro_original = criarTabuleiro(linhas, colunas);
            if (!tabuleiro || !tabuleiro_original) {
        ...
        if (opcao == 3) {
            ...
            printf("f - refazer o último comando desfeito\n");
            ...
            if (fgets(linhaInput, sizeof(linhaInput), stdin) != NULL) {
                ...
                if (strcmp(linhaInput, "0") == 0) {
                    ...
                    if (historicoPos > 0) {
                        ...
                        historicoPos--;
                        copiarTabuleiro(tabuleiro, historicoTabuleiros[historicoPos], linhas, colunas);
                        strcpy(mensagem, "Jogada desfeita.");
                    } else {
                        strcpy(mensagem, "Nada para desfazer.");
                    }
                } else if (strcmp(linhaInput, "f") == 0 || strcmp(linhaInput, "F") == 0) {
                    if (historicoPos < historicoCount - 1) {
                        historicoPos++;
                        copiarTabuleiro(tabuleiro, historicoTabuleiros[historicoPos], linhas, colunas);
                        strcpy(mensagem, "Jogada refeita.");
                    } else {
                        strcpy(mensagem, "Nada para refazer.");
                    }
